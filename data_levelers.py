...
class Distributor(LogicBlock):
    ...
    def break_up(self, degree: int, extra_splitters: set, dependency_list: list):
        ...
        for item in basket:
            ...
            extra_splitters.add(Distributor(self.parent, sibling, internal_wires))
...
class DataLeveler:
    ...
    def insert_virtual_splitters(self):
        ...
        print("\n max delay after inserting virtual splitters of any fan-outs is : {}".format(self.module.delay))
    ...
    def _break_splitter(self, vs: Distributor):
        ...
        for out in vs.output_pins:
            ...
            if len(net.post) != 0:
                ...
                #     wire_out = instance.wire_out
                #     if isinstance(wire_out, PortOut):
                #         delays = self.module.delays
                #         margin = max(delays.values()) - delays.get(wire_out.name)
                # else:
        ...
        vs.splitter_submerge(dependency)
        self.virtual_splitters.update(dependency)
        for d in dependency:
            d.break_up(self.cell.max_fan_out, self.virtual_splitters, dependency)
